...
app.config['SQLALCHEMY_DATABASE_URI'] = 'mysql://root:root@localhost:3306/dutylist2'
db = SQLAlchemy(app)
...
def register():
    ...
    if request.method == "POST":
        ...
        if username or phone or password or repassword:
            ...
            print 'data.username = %s' %data.username
            print 'data.phone = %s' %data.phone
            print 'data.password = %s' %data.password
            print 'data.time.time() = %s' %data.create_time
            print 'data.user_id = %s' %data.user_id
